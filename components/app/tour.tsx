...
function stepsFor(path: string, t: TourCtx): DriveStep[] {
  ...
  if (path === "/map") {
    const steps: DriveStep[] = [
      ...
      { element: el("panel"), popover: { title: "필지 패널", description: "기본정보(결측은 \"정보없음\"), 필지 여건, 필지 타임라인(사용승인·위반 표기 변화·사고 보도·착공·급경사지), 주변 현황. 박달동 139-137은 2026-05-21 축대 붕괴 이력이 있고 공개 급경사지 목록에는 없습니다." } },
      { element: el("facts"), popover: { title: "필지 여건 — 안양시 공공데이터", description: "행정동·수방자재(안양시)·개발제한구역(브이월드)·급경사지(행안부)·공공건축물·대피/급수시설·공동주택 단지·착공신고(안양시)·이웃 위반을 반경으로 대조하고 적용 법령을 붙입니다. 항목마다 출처·기준일이 있습니다." } },
    ];
    if (t.officer) {
      steps.push(
