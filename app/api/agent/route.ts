...
규칙:
1. 필지 속성·점수·이력·조문은 반드시 도구(parcel_lookup, signal_score, timeline_build, rules_rag, doc_render, doc_check)로 조회한 결과만 쓴다. 도구 결과에 없는 수치·사건·날짜를 만들지 않는다.
...
9. 사용자가 문서를 요청하면 doc_render 를 호출한다(survey_plan: 조사 목록, prior_notice: 선택 필지). 반환된 checklist 의 누락 항목을 답변에 적는다.
...
export async function POST(req: NextRequest) {
  ...
  const ctxLine = [
    ...
    ctx.verdicts && Object.keys(ctx.verdicts).length ? `판정 보유 ${Object.keys(ctx.verdicts).length}건` : "판정 없음",
  ].filter(Boolean).join(" · ");
